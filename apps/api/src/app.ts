...
export async function buildApp() {
  const app=Fastify({logger:{level:process.env.LOG_LEVEL??'info',redact:['req.headers.authorization','req.headers.cookie','*.password','*.passwordHash','*.email','*.name','*.token','*.resetToken','*.refreshToken','*.parentEmail']}}).withTypeProvider<ZodTypeProvider>();
  ...
  await app.register(helmet,{contentSecurityPolicy:{directives:{defaultSrc:["'self'"],scriptSrc:["'self'"],styleSrc:["'self'","'unsafe-inline'"],imgSrc:["'self'",'data:'],fontSrc:["'self'"],connectSrc:["'self'",...env.CORS_ORIGIN.split(',').map(o=>o.trim()).filter(Boolean),env.API_BASE_URL],frameAncestors:["'self'",'https://web.telegram.org'],formAction:["'self'"],baseUri:["'none'"],objectSrc:["'none'"]}},strictTransportSecurity:{maxAge:31536000,includeSubDomains:true,preload:true},referrerPolicy:{policy:'strict-origin-when-cross-origin'},xFrameOptions:false,xContentTypeOptions:true,hidePoweredBy:true});
  ...
  await app.register(async(api:TypedFastify)=>{await api.register(healthRoutes);await api.register(telegramRoutes);await api.register(authRoutes,{prefix:'/auth'});await api.register(meRoutes);await api.register(onboardingRoutes);await api.register(topicsRoutes);await api.register(dashboardRoutes);await api.register(voiceRoutes);await api.register(sessionRoutes);await api.register(roundRoutes)},{prefix:'/api/v1'});
  return app;
}
